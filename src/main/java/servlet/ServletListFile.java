 ...
 * //获取上传文件的目录 String
 * uploadFilePath=this.getServletContext().getRealPath("/WEB-INF/upload");
 * //存储要下载的文件名 Map<String,String> fileNameMap=new HashMap<String, String>();
 * 
 * 递归遍历filepath目录下的所有文件和目录，将文件的文件名存储到map集合中
 * 
 * //file既可以代表一个文件也可以代表一个目录 listFile(new File(uploadFilePath),fileNameMap);
 * //将map集合发送到listFile.jsp页面进行显示 request.setAttribute("fileNameMap",
 * fileNameMap);
 * request.getRequestDispatcher("admin/fileList.jsp").forward(request,
 * response); }
 * 
 * private void listFile(File file, Map<String, String> fileNameMap) {
 * //如果file代表的不是一个文件，而是一个目录 if(!file.isFile()){ //列出该目录下的所有文件和目录 File
 * files[]=file.listFiles(); //遍历files数组 for(File f:files){ //递归
 * listFile(f,fileNameMap); } }else{
 *//**
